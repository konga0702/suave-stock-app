-- =============================================
-- 011: SCHEDULED ステータス対応
-- - sync_inventory_transaction で OUT/SCHEDULED を受け付ける
-- - SCHEDULED は transaction のみ作成し在庫個体は変更しない
-- =============================================
...
DECLARE
  ...
  v_first_requested_quantity int;
BEGIN
  v_source := nullif(
    trim(
      COALESCE(
        p_payload->>'source',
        p_payload->>'external_source',
        ''
      )
    ),
    ''
  ...
  IF v_status IS NULL THEN
    v_status := 'COMPLETED';
  END IF;
  IF v_status NOT IN ('COMPLETED', 'SCHEDULED') THEN
    RETURN jsonb_build_object(
      'ok', false,
      'error', jsonb_build_object('code', 'VALIDATION_ERROR', 'message', 'status must be COMPLETED or SCHEDULED')
    );
  END IF;

  v_is_scheduled := (v_status = 'SCHEDULED');

  IF v_is_scheduled AND v_type <> 'OUT' THEN
    RETURN jsonb_build_object(
      'ok', false,
      'error', jsonb_build_object('code', 'VALIDATION_ERROR', 'message', 'status SCHEDULED is only supported for OUT transactions')
    );
  END IF;
  ...
  BEGIN
    ...
    IF v_is_scheduled THEN
      SELECT sl.product_code, sl.quantity
      INTO v_first_product_code, v_first_requested_quantity
      FROM _sync_lines sl
      ORDER BY sl.line_no
      LIMIT 1;
      ...
      RETURN jsonb_build_object(
        ...
        'mode', 'scheduled',
        'created', true,
        'product_code', v_first_product_code,
        'requested_quantity', v_first_requested_quantity,
    ...
    RETURN jsonb_build_object(
      'ok', true,
      'mode', CASE WHEN v_is_scheduled THEN 'scheduled' ELSE 'out_created' END,
      'transaction_id', v_tx_id,
      'external_source', v_source,
      'external_id', v_external_id,
      'created', true
